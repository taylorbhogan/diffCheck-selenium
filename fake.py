...
def executeDiffChecking():
    def _handleFailure(driver, file_name, sheet_index, error=""):
        ...
        driver.get_screenshot_as_file(f'./screenshots/{file_name}/{time.time_ns()}-{os.path.splitext(file_name)[0]}\
                                      -sheetIdx-{sheet_index}{error}.png')
    ...
    originalExcelDocuments = sorted(list(filter(
        lambda val: os.fsdecode(os.path.splitext(val)[1]) == '.xlsx', os.listdir(originalDirectoryPath))))
    changedExcelDocuments = sorted(list(filter(
        lambda val: os.fsdecode(os.path.splitext(val)[1]) == '.xlsx', os.listdir(changedDirectoryPath))))
    ...
    for idx in range(len(originalExcelDocuments)):  # each Excel doc
        ...
        sheetNameSelects = WebDriverWait(driver, 15).until(
            EC.presence_of_all_elements_located((By.XPATH,
                                                 "//select[@class='excel-input_sheetSelect__p7MmN diffResult']"))
        )
        sheet_idx = 1
        ...
        if len(optionsLeft) == len(optionsRight):
            ...
            _handleFailure(driver=driver,
                           file_name=os.fsdecode(changedExcelDocuments[idx]),
                           sheet_index=sheet_idx,
                           error="NUM SHEETS MISMATCH")
